...
const Header = ({ searchPlaceholder }) => {
	...
	useEffect(() => {
		...
		return () =>
			window.removeEventListener("mousedown", handleCalenderClick);
	...
	const { data, isLoading } = useQuery(
		...
		() =>
			fetchApi(
				`/auto-complete?query=${debouncedSearchInput}&hitsPerPage=6`
			),
		{
			enabled:
